const validatorCpfCnpj = (val) => {
  ...
  // Clean CPF/CNPJ
  const claenCpfCnpj = (cpf_cnpj) => {
    let characters = cpf_cnpj.trim();
  ...
  const checkEquality = (cpf_cnpj) => {
    let characters = cpf_cnpj.split('');
  ...
  const validatesCnpj = (cnpj) => {
    ...
    let firstNumberCnpj = cnpj.substring(0, 12);

    // Do the first calculation
    let firstCalculation = calcDigitsPositions(firstNumberCnpj, 5);

    // The second calculation is the same as the first, however, it starts at position 6
    let secondCalculation = calcDigitsPositions(firstCalculation, 6);

    // Concatenates the second digit with the CNPJ
    let newCnpj = secondCalculation;
